...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (_, announcements) = analysis::parse::just_parse::<analysis::parse::Announcements>(parsing_demo)?;
    println!("demo-parsed: {:?}", announcements);

    let filename = std::env::args().nth(1)
        .ok_or("Использование: cli <путь_к_файлу_логов>")?;
    println!("Trying opening file '{}' from directory '{}'",
        filename, std::env::current_dir()?.to_string_lossy());
