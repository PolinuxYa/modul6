...
trait Parser {
    type Dest;
    fn parse<'i>(&self, input: &'i str) -> Result<(&'i str, Self::Dest), ()>;
}
...
#[error("не удалось распарсить ввод")]
pub struct ParseError;

/// Публичный трейт для типов, которые можно десериализовать через [`just_parse`].
/// Реализован автоматически для всех типов, реализующих внутренний `Parsable`.
pub trait JustParsable: Sized {
    fn parse_str<'i>(input: &'i str) -> Result<(&'i str, Self), ParseError>;
}
impl<T: Parsable> JustParsable for T {
    fn parse_str<'i>(input: &'i str) -> Result<(&'i str, Self), ParseError> {
...
mod stdp { // parsers for std types
    ...
    impl Parser for U32 {
        type Dest = u32;
        fn parse<'i>(&self, input: &'i str) -> Result<(&'i str, Self::Dest), ()> {
            let (remaining, is_hex) = if let Some(rest) = input.strip_prefix("0x") {
                (rest, true)
            } else {
                (input, false)
            };
            ...
            let value = std::num::NonZeroU32::new(value).ok_or(())?.get(); // в наших логах нет нулей
    ...
    impl Parser for I32 {
        ...
        fn parse<'i>(&self, input: &'i str) -> Result<(&'i str, Self::Dest), ()> {
            ...
            let value = std::num::NonZeroI32::new(value).ok_or(())?.get(); // в наших логах нет нулей
...
fn do_unquote<'i>(input: &'i str) -> Result<(&'i str, String), ()> {
    let mut result = String::new();
    let mut escaped_now = false;
    let after_quote = input.strip_prefix('"').ok_or(())?;
...
fn do_unquote_non_escaped<'i>(input: &'i str) -> Result<(&'i str, String), ()> {
...
impl<T: Parser> Parser for List<T> {
    ...
    fn parse<'i>(&self, input: &'i str) -> Result<(&'i str, Self::Dest), ()> {
        ...
        loop {
            ...
            remaining = new_remaining.trim_start().strip_prefix(',').ok_or(())?.trim_start();
            result.push(item);
        }
    }
}
...
/// Парсит входную строку в любой тип, реализующий [JustParsable]
pub fn just_parse<'i, T: JustParsable>(input: &'i str) -> Result<(&'i str, T), ParseError> {
    T::parse_str(input)
}
...
impl Parsable for LogLine {
    ...
    fn parser () -> Self::Parser {
        map(
            ...
            // stdp::U32 уже отвергает 0, поэтому unwrap безопасен
            |(kind, request_id)| LogLine {
                kind,
                request_id: std::num::NonZeroU32::new(request_id).unwrap(),
            }
        )
    }
}
...
pub fn parse_log_line<'i>(input: &'i str) -> Result<(&'i str, LogLine), ParseError> {
    LogLine::parser().parse(input).map_err(|_| ParseError)
}
...
mod test {
    ...
    #[test]
    fn test_u32() {
        assert_eq!(stdp::U32.parse("411"), Ok(("".into(), 411)));
        assert_eq!(stdp::U32.parse("411ab"), Ok(("ab".into(), 411)));
        assert_eq!(stdp::U32.parse(""), Err(()));
        assert_eq!(stdp::U32.parse("-3"), Err(()));
        assert_eq!(stdp::U32.parse("0x03"), Ok(("".into(), 0x3)));
        assert_eq!(stdp::U32.parse("0x03abg"), Ok(("g".into(), 0x3ab)));
        assert_eq!(stdp::U32.parse("0x"), Err(()));
    }

    #[test]
    fn test_i32() {
        assert_eq!(stdp::I32.parse("411"), Ok(("".into(), 411)));
        assert_eq!(stdp::I32.parse("411ab"), Ok(("ab".into(), 411)));
        assert_eq!(stdp::I32.parse(""), Err(()));
        assert_eq!(stdp::I32.parse("-3"), Ok(("".into(), -3)));
    ...
    fn test_strip_whitespace() {
        assert_eq!(strip_whitespace(tag("hello")).parse(" hello world"), Ok(("world".into(), ())));
        assert_eq!(strip_whitespace(tag("hello")).parse("hello"), Ok(("".into(), ())));
        assert_eq!(strip_whitespace(stdp::U32).parse(" 42 answer"), Ok(("answer".into(), 42)));
    }

    #[test]
    fn test_delimited() {
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32]"), Ok(("".into(), 0x32)));
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]".into())).parse("[0x32] nice"), Ok((" nice".into(), 0x32)));
    ...
    #[test]
    fn test_list() {
        assert_eq!(list(stdp::U32).parse("[1,2,3,4,]"), Ok(("".into(), vec![1, 2, 3, 4,])));
        assert_eq!(list(stdp::U32).parse(" [ 1 , 2 , 3 , 4 , ] nice"), Ok(("nice".into(), vec![1, 2, 3,4,])));
        assert_eq!(list(stdp::U32).parse("1,2,3,4,"), Err(()));
        assert_eq!(list(stdp::U32).parse("[]"), Ok(("".into(), vec![])));
    }
    ...
    #[test]
    fn test_asset_dsc() {
        assert_eq!(all2(strip_whitespace(tag("AssetDsc")),strip_whitespace(tag("{"))).parse(" AssetDsc { "), Ok(("".into(), ((),()))));
